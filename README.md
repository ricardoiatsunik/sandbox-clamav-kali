# Sandbox Offline com ClamAV no Kali Linux (VirtualBox)

Este repositório documenta a criação de um ambiente sandbox offline utilizando o Kali Linux em uma máquina virtual no VirtualBox, com foco em análise estática de arquivos suspeitos usando o antivírus ClamAV. O objetivo é garantir segurança e isolamento completo durante a análise.

## Ferramentas utilizadas

- ClamAV
- Kali Linux (máquina virtual)
- VirtualBox
- Snapshots e clones
- FreshClam (atualizador de assinaturas do ClamAV)

## Passos realizados

1. Instalação do Kali Linux via VirtualBox.
2. Desativação do compartilhamento de pastas e integrações com o host.
3. Criação de um clone limpo da máquina virtual para preservar um estado base seguro.
4. Criação de um snapshot antes da análise para permitir reversão após uso.
5. Atualização das definições do ClamAV com `freshclam` antes de desconectar a rede.
6. Desativação da interface de rede da VM no VirtualBox (modo totalmente offline).
7. Execução da varredura com `clamscan` no arquivo ou diretório suspeito.
8. Reversão ao snapshot após a análise para garantir a integridade do ambiente.

## Comando básico do ClamAV

Para escanear arquivos ou diretórios:

```
clamscan -r /caminho/do/arquivo_ou_diretorio
```

Opções:
- `-r`: escaneia recursivamente diretórios.

## Boas práticas de segurança

- Nunca analise arquivos suspeitos com a máquina conectada à internet.
- Utilize snapshots e clones para garantir que qualquer alteração seja revertida.
- Atualize as definições do ClamAV com `freshclam` enquanto a máquina ainda está online, antes de desconectar.

## Estrutura recomendada do projeto

```
sandbox-clamav-kali/
├── README.md
├── scripts/
│   └── exemplo_clamscan.sh
├── notas/
│   └── configuracao_virtualbox.md
```
